<section id="testimoni" class="py-24 bg-white">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="bg-[#0A2B7A] rounded-3xl p-12 lg:p-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[600px]">
            ...
            <div class="relative w-full h-full min-h-[350px]" id="testimonial-cards">
                <!-- Javascript will populate this -->
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function render() {
        ...
        testimonials.forEach(t => {
            ...
            const bgClass = isActive ? 'bg-white/10 ring-1 ring-white/20 shadow-lg' : 'hover:bg-white/5 opacity-60 hover:opacity-100';
            
            const div = document.createElement('div');
            div.className = `flex items-center gap-4 p-4 rounded-xl cursor-pointer transition-all duration-300 ${bgClass}`;
            div.innerHTML = `
                <img src="${t.avatar}" alt="${t.name}" class="h-14 w-14 rounded-full border-2 border-white/20">
                <div>
                    <p class="text-lg text-slate-200 font-medium">${t.short_quote}</p>
                    <p class="text-sm text-blue-300 mt-1 font-semibold">
                        ${t.name} <span class="text-blue-400 font-normal opacity-70 ml-1">- ${t.role}</span>
                    </p>
                </div>
            `;
            
            div.addEventListener('mouseover', () => { if(activeId !== t.id) { activeId = t.id; render(); } });
            div.addEventListener('click', () => { if(activeId !== t.id) { activeId = t.id; render(); } });
        ...
        if (t) {
            ...
            card.className = 'bg-[#153B94] rounded-2xl p-8 lg:p-10 shadow-2xl border border-white/10 absolute inset-0 flex flex-col justify-between animate-[fadeIn_0.5s_ease-out]';
            ...
            t.stats.forEach(stat => {
                statsHtml += `
                    <div>
                        ...
                        <p class="text-[10px] font-bold tracking-wider text-blue-300 uppercase mt-1">${stat.label}</p>
                    </div>
                `;
            });
