    ...
    <section class="relative py-16 overflow-hidden bg-slate-50 min-h-screen">
        ...
        <div class="relative z-10 mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <a href="{{ route('gallery') }}"
                class="inline-flex items-center text-sm font-semibold text-slate-600 hover:text-blue-600 transition mb-8 group">
            ...
            <div class="rounded-3xl overflow-hidden bg-white shadow-xl ring-1 ring-slate-200 mb-10">
                ...
                <div class="aspect-video w-full bg-gradient-to-br from-slate-100 to-slate-200 relative overflow-hidden">
                    <img src="{{ asset('storage/' . $project->thumbnail_path) }}" alt="{{ $project->title }}"
                        class="w-full h-full object-cover" />
                </div>
                @else
                <div
                    class="aspect-[3/1] w-full bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center">
                    <span class="text-7xl font-extrabold text-white/30 tracking-wider select-none">{{
                        strtoupper(substr($project->student->user->name ?? '', 0, 2)) }}</span>
                </div>
                @endif
                ...
                <div class="p-8 sm:p-10">
                    <div class="flex flex-wrap items-center gap-3 mb-4">
                        <span
                            class="inline-flex items-center rounded-lg bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700 ring-1 ring-emerald-600/20">
                        ...
                        <span
                            class="inline-flex items-center rounded-lg bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700 ring-1 ring-blue-600/20">{{
                            ucfirst($project->development_model) }}</span>
                        @if($project->validation)
                        @php $avgScore = number_format(($project->validation->functionality_score +
                        $project->validation->code_quality_score + $project->validation->documentation_score +
                        $project->validation->originality_score) / 4, 1); @endphp
                        <span
                            class="inline-flex items-center rounded-lg bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700 ring-1 ring-amber-600/20">
                    ...
                    <div class="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100">
                        <div
                            class="h-11 w-11 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-lg overflow-hidden">
                            <x-avatar :url="$project->student->user->avatar_url"
                    ...
                    <div class="mb-8">
                        <a href="{{ $project->github_url }}" target="_blank"
                            class="inline-flex items-center space-x-2 text-sm font-semibold text-slate-700 bg-white border border-slate-300 px-5 py-2.5 rounded-xl hover:bg-slate-50 transition shadow-sm">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <span
                                class="inline-flex items-center rounded-lg bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 ring-1 ring-inset ring-indigo-700/10">{{
                                $tech }}</span>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-6 sm:p-8 mb-8">
                <h2 class="text-lg font-bold text-slate-900 mb-4 flex items-center">
                    <svg class="w-5 h-5 mr-2 text-slate-500" viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                    </svg>
                    Statistik GitHub
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div class="bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
                        <div class="text-2xl font-bold text-slate-900">{{ $project->githubMetadata->commit_count ?? '-'
                            }}</div>
                        <div class="text-xs text-slate-500 font-medium mt-1">Total Commit</div>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
                        <div class="text-2xl font-bold text-slate-900">{{ $project->githubMetadata->language ?? '-' }}
                        </div>
                ...
                <p class="text-xs text-slate-500 mt-3">Commit terakhir: {{
                    \Carbon\Carbon::parse($project->githubMetadata->last_commit_at)->diffForHumans() }} — <span
                        class="text-slate-700 italic">"{{ Str::limit($project->githubMetadata->last_commit_message, 80)
                        }}"</span></p>
                @endif
            </div>
            @endif
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-6 sm:p-8 mb-8">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                    <div class="bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
                        <div class="text-2xl font-bold text-slate-900">{{ $project->validation->functionality_score ??
                            '-' }}</div>
                        <div class="text-xs text-slate-500 font-medium mt-1">Fungsionalitas</div>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
                        <div class="text-2xl font-bold text-slate-900">{{ $project->validation->code_quality_score ??
                            '-' }}</div>
                        <div class="text-xs text-slate-500 font-medium mt-1">Kualitas Kode</div>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
                        <div class="text-2xl font-bold text-slate-900">{{ $project->validation->documentation_score ??
                            '-' }}</div>
                        <div class="text-xs text-slate-500 font-medium mt-1">Dokumentasi</div>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-4 text-center border border-slate-100">
                        <div class="text-2xl font-bold text-slate-900">{{ $project->validation->originality_score ?? '-'
                            }}</div>
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-6 sm:p-8 mb-8">
                ...
                <div class="space-y-2">
                    ...
                    <a href="{{ route('documentation.download', $doc) }}" target="_blank"
                        class="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3.5 hover:bg-slate-50 transition group">
                        <div class="flex items-center gap-3 min-w-0">
                            @php
                            $docColors = ['pdf' => 'text-rose-500', 'video' => 'text-violet-500', 'image' =>
                            'text-blue-500', 'presentation' => 'text-amber-500', 'spreadsheet' => 'text-emerald-500',
                            'other' => 'text-slate-500'];
                            @endphp
                            <div
                                class="flex-shrink-0 h-10 w-10 rounded-lg bg-slate-100 flex items-center justify-center">
                                <svg class="w-5 h-5 {{ $docColors[$doc->doc_type] ?? 'text-slate-500' }}" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-slate-500">{{ ucfirst($doc->doc_type) }} &bull; {{
                                    number_format($doc->file_size / 1024, 0) }} KB{{ $doc->description ? ' — ' .
                                    $doc->description : '' }}</p>
                            </div>
                        </div>
                        <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-600 transition flex-shrink-0"
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 p-6 sm:p-8 mb-8">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ $url->url }}" target="_blank"
                        class="inline-flex items-center gap-2 rounded-xl bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-700 border border-slate-200 hover:bg-white hover:border-blue-200 transition shadow-sm">
